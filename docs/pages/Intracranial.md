---
lang: 'en'
slug: '/009C77'
---

## Definition

[Old Someday](./../.././docs/pages/Old%20Someday.md) [Project](./../.././docs/pages/Project.md). [Infinite Canvas](./../.././docs/pages/Infinite%20Canvas.md) for Zettlekasten.

- [FIRE Engine](./../.././docs/pages/FIRE%20Engine.md)
- Aim to implement Zettlekasten
- Uses
  - PencilKit
  - [Atomic Notes](./../.././docs/pages/Atomic%20Notes.md)
  - [Infinite Canvas](./../.././docs/pages/Infinite%20Canvas.md)
- Notes can refer to notes (like Hepta)
- Can view relation graphs
- Gesture Recognition: draw a certain shape to tag/link notes

## Problem

- After using Zettlekasten for a while, taking notes as document files kinda suck.
- I want to take notes with my iPad.

## Prodrafts

- I was using [Prodrafts](https://apps.apple.com/us/app/prodrafts/id1545810067), but it has limits:
  - 👍 [Infinite canvas](./../.././docs/pages/Infinite%20Canvas.md)
  - 👍 Native PencilKit support
  - 👍 [Apple](./../.././docs/pages/Apple.md)-ish design language
  - 👎 Unable to construct Zettlekasten
    - 👎 No graphing
    - 👎 No backlinking
    - 👎 No referencing
  - 👎 Poor [i18n](./../.././docs/pages/Internationalization.md) support (initially targeted for [Chinese](./../.././docs/pages/%EC%A4%91%EA%B5%AD.md) users.)
    - 👎 Significant portion is untranslated

## Reference

- [2022-07-26](./../.././docs/journals/2022-07-26.md)
