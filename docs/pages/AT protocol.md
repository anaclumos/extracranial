...
1. **Identity:** Users in the AT Protocol are identified by domain names, which [map](./../.././docs/pages/Map.md) to cryptographic URLs. This ensures security for the user's account and data.
...
3. **Federation:** The protocol operates on a federated networking model to maintain usability and reliability. It uses standard web technologies like [HTTP](./../.././docs/pages/HTTP.md) and WebSockets to synchronize data between servers.
4. **Interoperation:** The AT Protocol uses a global schemas network known as Lexicon to enable [interoperability](./../.././docs/pages/Interoperability.md) across servers, allowing software from different organizations to understand each other's data without needing to exchange rendering code.
...
AT Protocol is not a [blockchain](./../.././docs/pages/Blockchain.md) nor uses [blockchain](./../.././docs/pages/Blockchain.md) technology. It's also different from existing solutions like [ActivityPub](./../.././docs/pages/ActivityPub.md) due to a focus on portability and other unique design principles. The AT Protocol utilizes its formats like Lexicon and XRPC over [alternatives](./../.././docs/pages/Alternative.md) like [Resource Description Framework](./../.././docs/pages/Resource%20Description%20Framework.md) and [JSON-LD](./../.././docs/pages/JSON-LD.md) due to better developer experience and usability. These tools are designed to make distributed systems development easier and more robust.
