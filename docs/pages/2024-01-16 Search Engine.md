...
[2024-01-16](./../.././docs/journals/2024-01-16.md)
...
## [Google Pizza Box](./../.././docs/pages/Google%20Pizza%20Box.md)

<figure>

![09853B.png](./../.././docs/assets/09853B.png)
...
## Web [Crawling](./../.././docs/pages/Crawler.md) Issues
...
## [Cohere Rerank](./../.././docs/pages/Cohere%20Rerank.md)

See content.

## Simplest [Crawler](./../.././docs/pages/Crawler.md) Lifecycle
...
- Challenges
  - need to filter spam; or spider traps (A spider trap, also known as a [crawler](./../.././docs/pages/Crawler.md) trap, is a set of web pages that can cause a web [crawler](./../.././docs/pages/Crawler.md) or search bot to enter an infinite loop or other recursive situation, wasting its resources and affecting the [crawler](./../.././docs/pages/Crawler.md)'s ability to explore a website.)
...
## [Crawling](./../.././docs/pages/Crawler.md) Strategies

- BFS [crawling](./../.././docs/pages/Crawler.md) brings in high-quality pages early in the crawl
- Adding links to the end of the queue (FIFO) results in a BFS
- Adding them to the front of the queue (LIFO) yields a DFS
- Prioritizing links in a certain way can create a "focused [crawler](./../.././docs/pages/Crawler.md)" that targets "interesting" pages, such as
...
A spider trap occurs when a [crawler](./../.././docs/pages/Crawler.md) repeatedly visits the same web page. One of the most common types of spider traps is created using session IDs, which J2EE, ASP, .NET, and PHP manage. Session IDs are often used to keep track of visitors, and some websites put a unique ID in the URL. Each user gets a unique ID, and it's often requested from each page. The problem arises when Googlebot visits the page, spiders it, and then leaves. It then goes to another page and finds a link to the previous page, but since it has been given a different session ID, the link shows up as another URL. To avoid such traps, one way is for the [crawler](./../.././docs/pages/Crawler.md) to be cautious when the query string "ID=" is present in the URL. Another technique is to monitor the URL length and stop if the length becomes too long.
...
The second generation of spam web pages used a technique called cloaking. When a web server detected a request from a [crawler](./../.././docs/pages/Crawler.md), it returned a different page than the one returned from a user request. This caused the page to be mistakenly indexed.
...
## Measuring and Tuning a [Crawler](./../.././docs/pages/Crawler.md)

Improving the performance of a [crawler](./../.././docs/pages/Crawler.md) essentially involves three main aspects: enhancing the speed of URL parsing, improving network bandwidth speed, and enhancing fault tolerance. There are several other issues that may arise, such as determining how often to restart the process (refresh strategies), detecting duplicate pages, identifying mirror sites, accelerating DNS lookup, normalizing URLs, and handling malformed HTML.
...
3. Customize the [crawler](./../.././docs/pages/Crawler.md). Allow the issuance of many resolution requests simultaneously by implementing many DNS resolvers. This will enable the [crawler](./../.././docs/pages/Crawler.md) to handle multiple requests efficiently.

## Multi-Threaded [Crawling](./../.././docs/pages/Crawler.md)
...
## Distributed [Crawling](./../.././docs/pages/Crawler.md) Approaches

After optimizing the [crawler](./../.././docs/pages/Crawler.md) program, the next step is to determine how many crawlers will run simultaneously. Two scenarios can be considered.

In scenario 1, a centralized [crawler](./../.././docs/pages/Crawler.md) controls a set of parallel crawlers, which run on a local area network. A parallel [crawler](./../.././docs/pages/Crawler.md) comprises multiple [crawling](./../.././docs/pages/Crawler.md) processes that communicate through the network, also known as an intra-site parallel [crawler](./../.././docs/pages/Crawler.md).
...
## Implications for a Web [Crawler](./../.././docs/pages/Crawler.md)

Search engines usually use multiple crawlers that run constantly without interruption. When a [crawler](./../.././docs/pages/Crawler.md) replaces an old version of a page with a new one, there are two ways to do it - in-place or shadowing.
