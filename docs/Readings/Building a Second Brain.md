...
## 방법론
- 저자는 이를 [CODE](../Interests/CODE.md)]라고 칭한다.
...
### 정리 (Organize)
- 적당히 정렬한다.
- 컴퓨터과학의 데이터 구조론과 비슷했다. 너무 정렬을 안 하면 탐색에 오래 걸리고, 너무 탐색에 최적화하면 정렬에 오래 걸린다. 적당히 정렬하고 적당히 탐색하는 것이 중요하다.
- 세심한 창작을 위해 세심한 저장 구조가 필요하지 않다.
	- 예를 들어 세심하게 태그를 달면 너무 시간이 오래 걸린다.
- 저자는 [PARA](../Interests/PARA.md)라는 구조를 제안. 문서 참조.
- 개인적으로는 [PARA](../Interests/PARA.md) 구조에 대해 회의적이다.
	- Area와 Resource가 중복되는 느낌이 있다.
	- 나는 더 간단하게 Projects, Interests, 그리고 Reading으로 분리했다.
