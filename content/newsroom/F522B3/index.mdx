...
date: 2020-11-14
slug: '/F522B3'
description: 'Build a tiny calendar in vanilla JavaScript with functional rendering and zero post‑init DOM mutation: design, utilities, and code'
---
...
Note that we use `NUMBER_OF_DAYS_IN_WEEK` to remove [magic numbers](<./%3Chttps://en.wikipedia.org/wiki/Magic_number_(programming)%3E>) inside our code. It can be tough to decipher if we meet a random `7` during a code. Instead, using such constant increases the maintainability of the code.
...
### 3-2. Highlighting Saturday, Sunday, and Today
