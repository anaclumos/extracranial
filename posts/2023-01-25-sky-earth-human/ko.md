...
```py
def 입력(이전_상태, 활자_지도, 누른_키):

    마지막_한_글자 = 이전_상태[-1:]
    마지막_두_글자 = 이전_상태[-2:]

    # 마지막 두 글자를 새로운 값 활자_지도[누른_키][마지막_두_글자]로 변경
    if 마지막_두_글자 in 활자_지도[누른_키]:
        return 이전_상태[:-2] + 활자_지도[누른_키][마지막_두_글자]

    # 마지막 한 글자를 새로운 값 활자_지도[누른_키][마지막_한_글자]로 변경
    if 마지막_한_글자 in 활자_지도[누른_키]:
        return 이전_상태[:-1] + 활자_지도[누른_키][마지막_한_글자]

    return 이전_상태 + [활자_지도][누른_키][""]

    # 이게 정말 끝이다. 끝!
```

단 8줄 만으로 천지인 로직 구현이 가능하니 나는 감히 이것을 **전 세계에서 가장 단순한 천지인 구현체**라고 칭하겠다.
...
[활자 프로젝트는 모든 플랫폼에 탑재할 수 있도록 별도의 프로젝트로 분리해서 공개했다.](https://github.com/anaclumos/hwalja)
