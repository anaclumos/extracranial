export const backlinks = {
    "Apple": {
        "Swift": "- Programming Language for [[**Apple**]] platforms\n",
        "How is WebAssembly cross-platform when Assembly is not": "it pack multiple precompiled binary executables for multiple CPU vendors, like [[**Apple**]]'s Universal Binary?\n",
        "서울 프로": "본 목적은 Pretendard와 비슷하게 [[**Apple**]]의 [[San Francisco",
        "Screenshot as an API": " - You can copy objects from photos in the latest [[**Apple**]] iOS 16 update.\n",
        "Firefox": "it from spawning additional threads in the thread pools used by [[**Apple**]]'s libraries_\n",
        "San Francisco": "- Font by [[**Apple**]]\n",
        "Apple Silicon": "- Chips made by [[**Apple**]]\n",
        "Google": "one of the Big Five American information technology companies, alongside Amazon, [[**Apple**]], Meta, and Microsoft. [Google](https://en.wikipedia.org/wiki/Google)\n",
        "CalliFontia": "- [[**Apple**]] Neural Engine 적극 활용, 기기 자체에서 폰트 추론하기\n",
        "Letter to Mr. Matt Rickard on 2022-10-03": "That being said, Spotify and [[**Apple**]] Music have a market penetration of next to nothing.\n",
        "Person A480C6": "[[**Apple**]] also has a sound responsibility system — e.g., DRI.\n",
        "Person CC8297": "[[**Apple**]] truly respects its employees, and that comes from leadership positions.\n",
        "Intracranial": "  - 👍 [[**Apple**]]-ish design language\n",
        "Apple Earth": "- Google Earth app but with Smoo-ooth [[**Apple**]] Maps and iOS Widget Supports\n",
        "2022-10-13": "iPhone to be their next phone; 31% of teens own an [[**Apple**]] Watch: [Taking Stock With Teens® - Fall 2022](https://www.pipersandler.com/1col.aspx?id=6216)\n",
        "2022-06-21": "However, [[**Apple**]] Feedback Assistant is not a developer Q&A platform.\n",
        "2022-06-10": "  - [[**Apple**]] has [implemented pointer authentication](https://support.apple.com/guide/security/operating-system-integrity-sec8b776536b/1/web/1#sec0167b469d) on all of its custom ARM-based silicon",
        "2022-06-20": "Note that this is not [[**Apple**]] Passkey. This is the Privacy Pass standard. [[**Apple**",
        "2022-07-18": "iPad as a Tools for Thought feels good. Especially writing with [[**Apple**]] Pencil.\n"
    },
    "Pasted image 20220803235118.png": {
        "Swift": "![[**Pasted image 20220803235118.png**]]\n"
    },
    "Swift": {
        "SwiftUI": "- Declarative Framework made with [[**Swift**]]\n",
        "Apple": "- Creator of [[**Swift**]], [[**Swift**UI"
    },
    "Super App": {
        "Super App and Democracy": "says, “It's no coincidence at all that WeChat is the only “[[**Super App**|everything app]]” anyone can cite, and it comes from China, an authoritarian regime._\n",
        "Toss": "- I'd say _the most innovative_ Financial [[**Super App**]] I've ever seen in Korea and U.S.\n"
    },
    "Vercel": {
        "Satori": "and Web Worker|Web Workers. Building on top of the core engine, [[**Vercel**]] OG can be used inside Edge environments through [[WebAssembly",
        "Stories Behind Satori": "The [[**Vercel**]] team [[Dogfooding|dogfooded",
        "2022-08-28": "- I both used Cloudflare Pages and [[**Vercel**]] and concluded that [[**Vercel**",
        "2022-07-10": "- [[**Vercel**]] hid the compilation process with its proprietary `vercel dev` command.\n",
        "2022-06-22": "Deno envisions a [[**Vercel**]]-like experience for the backend (FaaS.)\n",
        "2022-09-30": "### [[**Vercel**]] [[NextJS",
        "2022-10-10": "  - I just love [[**Vercel**]]... they _know_ stuff.\n"
    },
    "WebAssembly": {
        "Satori": "core engine, Vercel OG can be used inside Edge environments through [[**WebAssembly**]] to create social card images easily.\n",
        "How is WebAssembly cross-platform when Assembly is not": "- What is the relationship between [[**WebAssembly**]] and [[Assembly",
        "Deno Deploy": "- TypeScript|TypeScript, [[**WebAssembly**|Wasm]], ES Modules\n",
        "Stories Behind Satori": "Using a [[**WebAssembly**|WASM]]-based approach also unlocked the possibility of running it on edge. [Introducing",
        "2022-06-14": "- [[**WebAssembly**|WASM]]과는 당장은 무관하다.\n",
        "2022-10-14": "- [[**WebAssembly**]]\n",
        "2022-10-15": "- [[**WebAssembly**]]: [paradust7/minetest-wasm: Experimental Minetest build for **WebAssembly**/Emscripten](https://github.com/paradust7/minetest-wasm)\n"
    },
    "HTML": {
        "Satori": "is needed. Using Vercel OG, you can define your images using [[**HTML**]] and [[CSS",
        "Tailwind": "> Rapidly build modern websites without ever leaving your [[**HTML**]]. A utility-first [[CSS",
        "CSS": "presentation of a document written in a markup language such as [[**HTML**]] or XML (including XML dialects such as [[SVG",
        "DOM": "is a cross-platform and language-independent interface that treats an XML or [[**HTML**]] document as a tree structure wherein each node is an object",
        "JavaScript": "one of the core technologies of the World Wide Web, alongside [[**HTML**]] and CSS. As of 2022, 98% of websites use JavaScript on",
        "JSX": "syntax familiar to many developers. It is similar in appearance to [[**HTML**]]. React components are typically written using JSX, although they do not",
        "Stories Behind Satori": "Given that SVG and [[**HTML**]] + [[CSS",
        "Markdown Email Client": "- Then... it sends as a beautiful and clean [[**HTML**]] email!\n",
        "2022-06-12": "rehydration is a technique in which client-side JavaScript|JavaScript converts a static [[**HTML**]] web page, delivered either through static hosting or server-side rendering, into"
    },
    "CSS": {
        "Satori": "- Tailwind [[**CSS**]] with the `tw` prop\n",
        "Tailwind": "Rapidly build modern websites without ever leaving your HTML. A utility-first [[**CSS**]] framework packed with classes like flex, pt-4, text-center, and rotate-90 that",
        "Font Features": "- [[**CSS**]] property to enhance legibilities of a [[Font",
        "Stories Behind Satori": "After simplifying the syntax with JSX and simple [[**CSS**]], Shu used [RazrFalcon/resvg](https://github.com/RazrFalcon/resvg) and [yisibl/resvg-js](https://github.com/yisibl/resvg-js) to convert the [[SVG",
        "Markdown Email Client": "- Before sending, select one [[**CSS**]]\n"
    },
    "Open Graph Image as a Service": {
        "Satori": "this for a while. [anaclumos/cho-sh-og-image: **Open Graph Image as a Service**](https://github.com/anaclumos/cho-sh-og-image). [[**Open Graph Image as a Service**]]\n",
        "2022-07-10": "longer make any improvements in `og-image.cho.sh`, and invest those time in [[**Open Graph Image as a Service**]].\n",
        "2022-07-18": "- Fiercely Overpriced for [[**Open Graph Image as a Service**]] could be public-key-encrypting query strings.\n",
        "2022-10-10": "- [[**Open Graph Image as a Service**]]\n"
    },
    "Emoji": {
        "Satori": "- Font and [[**Emoji**]] Subsets from Google Fonts and other CDNs\n",
        "Stories Behind Satori": "The same applies to [[**Emoji**|emojis]].\n",
        "TossFace Playground": "- Celebrating Redistribution of Korean and Futuristic [[**Emoji**|**Emoji**s]]. [유니코드 Private Use Area를 이용해 한국적, 시대적 가치를 담은 이모지 재배포"
    },
    "Tailwind": {
        "Satori": "- [[**Tailwind**]] [[CSS"
    },
    "Web Worker": {
        "Satori": "core engine, [Satori](https://github.com/vercel/satori), can be used in modern browsers, Node.js, and [[**Web Worker**|**Web Worker**s]]. Building on top of the core engine, [[Vercel",
        "WorkerDOM": "Google's take on providing DOM access to [[**Web Worker**]] Threads.\n",
        "Research": "   | Singular Noun     | _[[**Web Worker**]]_ (not _Workers_.) |\n",
        "Atomics": "- SharedArrayBuffer를 이용해 [[**Web Worker**]] 사이 메모리를 동기화하기 위해 사용됨.\n",
        "Stories Behind Satori": "Converting it into WebAssembly|WASM will also benefit by targeting [[**Web Worker**|**Web Worker**s]].\n",
        "iiframe": "- Improved `iframe` that containerizes an embedded web inside [[**Web Worker**|**Web Worker**s]].\n",
        "2022-07-25": "- [ ] Unfreeze the [[**Web Worker**]] thread with `Atomic.notify()`.\n",
        "2022-07-11": "Give this object to a [[**Web Worker**]] which includes third-party codes.\n",
        "2022-07-21": "  - Unfreeze the [[**Web Worker**]] thread with `Atomic.notify()`.\n",
        "2022-08-03": "iiframe — improved iframe — that specializes in containerizing iframes in [[**Web Worker**|**Web Worker**s]].\n",
        "2022-07-19": "  - [[**Web Worker**]] Thread에 Atomic Notify를 한다\n",
        "2022-06-22": "Preferably, WorkerDOM and [[**Web Worker**]] API will already have a _non-blocking operation_ feature.\n"
    },
    "Stories Behind Satori": {
        "Satori": "- [[**Stories Behind Satori**]]\n",
        "2022-10-13": "  - [x] [[**Stories Behind Satori**]]\n"
    },
    "Assembly": {
        "How is WebAssembly cross-platform when Assembly is not": "- What is the relationship between Web**Assembly** and [[**Assembly**]]?\n",
        "Inspecting Assembly": "- [[**Assembly**]]\n"
    },
    "Turing Machine": {
        "How is WebAssembly cross-platform when Assembly is not": "It follows how [[**Turing Machine**]]s would work in the real world.\n"
    },
    "Pasted image 20221012124224.png": {
        "How is WebAssembly cross-platform when Assembly is not": "![[**Pasted image 20221012124224.png**]]\n"
    },
    "Pasted image 20221012124422.png": {
        "How is WebAssembly cross-platform when Assembly is not": "![[**Pasted image 20221012124422.png**]]\n"
    },
    "assembly": {
        "How is WebAssembly cross-platform when Assembly is not": "- It is a specific purpose [[**assembly**]].\n",
        "Inspecting Assembly": "         | prints [[**assembly**]] codes          "
    },
    "Toss": {
        "Tossface": "- [[**Toss**]] once withdrew the emojis, but I submitted a request to put",
        "Toss Product Sans": "- Font by [[**Toss**]].\n"
    },
    "TossFace Playground": {
        "Tossface": "- [[**TossFace Playground**]]\n",
        "2022-08-18": "- [[**TossFace Playground**]]\n"
    },
    "Inspecting Assembly": {
        "Computer Systems": "- [[**Inspecting Assembly**]]\n",
        "GDB Commands": "- [[**Inspecting Assembly**]]\n",
        "2022-09-26": "- [[**Inspecting Assembly**]]\n",
        "2022-09-25": "- [[**Inspecting Assembly**]]\n"
    },
    "Karrot": {
        "Jira": "- Used at [[**Karrot**]]\n",
        "Height": "- Used at [[**Karrot**]]\n"
    },
    "FIRE": {
        "Software as a Service": "We expect we could attain [[**FIRE**]] through [[Software as a Service",
        "Fiercely Overpriced": "enough data out on how the real-world Software as a Service [[**FIRE**]] will work out.\n",
        "FIRE Engine": "- Software as a Service that drives revenue streams for [[**FIRE**]]\n"
    },
    "Software as a Service": {
        "Software as a Service": "We expect we could attain FIRE through [[**Software as a Service**]].\n"
    },
    "Venture Capital": {
        "Either be the best-in-class or the most efficient": "- [[**Venture Capital**|VC]]\n"
    },
    "Simple Analytics": {
        "Google Analytics": "- Alternative: [[**Simple Analytics**]], [[Plausible Analytics"
    },
    "Plausible Analytics": {
        "Google Analytics": "- Alternative: Simple Analytics, [[**Plausible Analytics**]]\n"
    },
    "Tools for Thought": {
        "Extracranial": "function while acting as a searchable Wikipedia. It usually tightly integrates [[**Tools for Thought**]] and Online platforms. For this system, I use [Docusaurus](https://docusaurus.io/) and [Obsidian](https://obsidian.md/).",
        "PKM": "Check [[**Tools for Thought**]] for PKM tools in the market.\n",
        "Research": "Concept | Plural Noun       | _[[**Tools for Thought**]]_           |\n",
        "Handwriting Tools for Thoughts": "- A specific [[**Tools for Thought**]] with handwriting.\n",
        "2022-10-13": "- [Visible – Make information useful](https://visible.page/): [[**Tools for Thought**]]\n",
        "2022-07-18": "- Obsidian-like [[**Tools for Thought**]]\n",
        "2022-07-07": "I see a repeated paradigm in the [[**Tools for Thought**]] [[Software as a Service"
    },
    "Svelte": {
        "Extracranial": "Seems like using [[**Svelte**]]\n"
    },
    "Pasted image 20221013120518.png": {
        "Extracranial": "![[**Pasted image 20221013120518.png**]]\n"
    },
    "Pasted image 20221013120525.png": {
        "Extracranial": "![[**Pasted image 20221013120525.png**]]\n"
    },
    "Pasted image 20221013120530.png": {
        "Extracranial": "![[**Pasted image 20221013120530.png**]]\n"
    },
    "Pasted image 20221013120747.png": {
        "Extracranial": "![[**Pasted image 20221013120747.png**]]\n"
    },
    "Pasted image 20221013120810.png": {
        "Extracranial": "![[**Pasted image 20221013120810.png**]]\n"
    },
    "Pasted image 20220925130955.png": {
        "Extracranial": "![[**Pasted image 20220925130955.png**]]\n"
    },
    "Pasted image 20220925131542.png": {
        "Extracranial": "![[**Pasted image 20220925131542.png**]]\n"
    },
    "Pasted image 20220925131410.png": {
        "Extracranial": "![[**Pasted image 20220925131410.png**]]\n"
    },
    "Pasted image 20220928232755.png": {
        "Extracranial": "![[**Pasted image 20220928232755.png**]]\n"
    },
    "E45BFC.png": {
        "Extracranial": "![[**E45BFC.png**]]\n"
    },
    "TypeScript": {
        "Svelte": "runtime, i.e., in the browser. The compiler itself is written in [[**TypeScript**]]. Its source code is licensed under MIT License and hosted on",
        "Deno Deploy": "- [[**TypeScript**|**TypeScript**]], [[WebAssembly|Wasm",
        "2022-07-20": "- JavaScript|JavaScript → [[**TypeScript**|**TypeScript**]]\n",
        "2022-09-30": "### [[**TypeScript**]] Dynamic Types with String Interpolations\n",
        "2022-07-06": "- Bundle, transpile, install and run JavaScript|JavaScript & [[**TypeScript**|**TypeScript**]] projects — all in Bun. Bun is a new JavaScript|JavaScript runtime"
    },
    "Satori": {
        "WebAssembly": "- [[**Satori**]]\n",
        "2022-10-11": "- [[**Satori**]]\n"
    },
    "San Francisco": {
        "서울 프로": "이름 또한 [[**San Francisco**|SF Pro]]에서 유래하였다.\n",
        "Font Features": "- [[**San Francisco**]], [[Inter",
        "Pretendard": "- Just like Toss Product Sans it is bizarrely similar to [[**San Francisco**]]\n",
        "Inter": "- bizarrely similar to [[**San Francisco**]], but it now has it's own style\n",
        "Toss Product Sans": "- Looks bizarrely similar to [[**San Francisco**]]\n",
        "2022-08-01": "it a lot — it combines the neatness and readability of [[**San Francisco**|SF Pro]] and the uniqueness of IBM Plex Sans.\n"
    },
    "Pretendard의 아쉬운 점": {
        "서울 프로": "- [[**Pretendard의 아쉬운 점**]]\n"
    },
    "Pasted image 20221014231601.png": {
        "Font Features": "![[**Pasted image 20221014231601.png**]]\n"
    },
    "Font": {
        "Font Features": "- CSS property to enhance legibilities of a [[**Font**]].\n",
        "Pretendard": "- [[**Font**]] by [orioncactus (Kil Hyung-jin)](https://github.com/orioncactus)\n",
        "Inter": "- [[**Font**]] by [rsms](https://rsms.me/)\n",
        "San Francisco": "- [[**Font**]] by [[Apple",
        "Toss Product Sans": "- [[**Font**]] by [[Toss"
    },
    "Inter": {
        "Font Features": "- San Francisco, [[**Inter**]], and [[Pretendard"
    },
    "Pretendard": {
        "Font Features": "- San Francisco, Inter, and [[**Pretendard**]] has awesome support.\n"
    },
    "Pasted image 20221014232128.png": {
        "Font Features": "![[**Pasted image 20221014232128.png**]]\n"
    },
    "Pasted image 20221013004704.png": {
        "Are.na": "![[**Pasted image 20221013004704.png**]]\n",
        "Jumpsite": "![[**Pasted image 20221013004704.png**]]\n"
    },
    "SVG": {
        "CSS": "language such as HTML or XML (including XML dialects such as [[**SVG**]], MathML, or XHTML). CSS is a cornerstone technology of the World",
        "Stories Behind Satori": "and simple CSS, Shu used [RazrFalcon/resvg](https://github.com/RazrFalcon/resvg) and [yisibl/resvg-js](https://github.com/yisibl/resvg-js) to convert the [[**SVG**]] image to [[PNG"
    },
    "Monolithic (Pure Hierarchy) Model": {
        "Pure Component Model": "- Alternative approach to [[**Monolithic (Pure Hierarchy) Model**]]\n",
        "Game Programming": "- [[**Monolithic (Pure Hierarchy) Model**]]\n",
        "2022-09-03": "- [[**Monolithic (Pure Hierarchy) Model**]]\n"
    },
    "초상권 vs 저작권": {
        "퍼블리시티권": "- [[**초상권 vs 저작권**]]\n",
        "CELLO": "- [[**초상권 vs 저작권**]]\n"
    },
    "Linux Commands": {
        "Internetworking": "- [[**Linux Commands**]]\n"
    },
    "GDB Commands": {
        "Internetworking": "- [[**GDB Commands**]]\n"
    },
    "Clock Skew Detected": {
        "Internetworking": "- [[**Clock Skew Detected**]]\n"
    },
    "HTTP": {
        "Internetworking": "- [[**HTTP**]]\n"
    },
    "Toss Product Sans": {
        "Pretendard": "- Just like [[**Toss Product Sans**]] it is bizarrely similar to [[San Francisco",
        "Toss": "- Creators of [[**Toss Product Sans**]] and [[Tossface",
        "2022-10-14": "- [[**Toss Product Sans**]]\n"
    },
    "Extracting String in Android": {
        "Android": "- [[**Extracting String in Android**]]\n"
    },
    "Android Equivalent of Div": {
        "Android": "- [[**Android Equivalent of Div**]]\n",
        "2022-09-17": "  - [[**Android Equivalent of Div**]]\n"
    },
    "Centering": {
        "Android": "- [[**Centering**]]\n",
        "2022-09-17": "  - [[**Centering**]]\n"
    },
    "Adding Space": {
        "Android": "- [[**Adding Space**]]\n",
        "2022-09-17": "  - [[**Adding Space**]]\n"
    },
    "Componentizing Views": {
        "Android": "- [[**Componentizing Views**]]\n",
        "2022-09-17": "  - [[**Componentizing Views**]]\n"
    },
    "Intracranial": {
        "Infinite Canvas": "- [[**Intracranial**]]\n",
        "Handwriting Tools for Thoughts": "- [[**Intracranial**]]\n",
        "2022-07-26": "- Will add this note to [[**Intracranial**]].\n"
    },
    "SwiftUI": {
        "Apple": "- Creator of Swift, [[**SwiftUI**]], [[Apple Silicon",
        "ULLO": "- PoC of [[**SwiftUI**]] Photo Widget App\n",
        "CELLO (Product)": "- [[**SwiftUI**]]\n",
        "Cube": "- [[**SwiftUI**]]\n",
        "2022-08-04": "### Explored [[**SwiftUI**]].\n"
    },
    "Apple Silicon": {
        "Apple": "- Creator of Swift, SwiftUI, [[**Apple Silicon**]]\n"
    },
    "Pasted image 20221013004357.png": {
        "Surfit": "![[**Pasted image 20221013004357.png**]]\n",
        "Jumpsite": "![[**Pasted image 20221013004357.png**]]\n"
    },
    "Google Analytics": {
        "GA": "- [[**Google Analytics**]]\n"
    },
    "We become what we do": {
        "Our tools also shape us": "- [[**We become what we do**]]\n",
        "2022-10-13": "- [[**We become what we do**]]\n"
    },
    "Roam Research": {
        "Our tools also shape us": "  - [[**Roam Research**]] makes you a connector + databaser\n",
        "Better Obsidian": "  - List view for Daily Notes like [[**Roam Research**]] and [[Logseq"
    },
    "Obsidian": {
        "Our tools also shape us": "  - [[**Obsidian**]] makes you a connector + writer\n",
        "Nota": "Awesome alternative to [[**Obsidian**]].\n",
        "Tools for Thought": "- [zadam/trilium](https://github.com/zadam/trilium): [[**Obsidian**]]-like note-taking service.\n",
        "Better Obsidian": "## Extension Ideas for [[**Obsidian**]]\n",
        "2022-10-13": "- [[**Obsidian**]] is now out of beta\n",
        "2022-07-18": "- [[**Obsidian**]]-like [[Tools for Thought"
    },
    "Letter to Mr. Matt Rickard on 2022-10-03": {
        "Screenshot as an API": "- I wrote a reply like the following. [[**Letter to Mr. Matt Rickard on 2022-10-03**]]\n",
        "2022-10-04": "- [[**Letter to Mr. Matt Rickard on 2022-10-03**]]\n"
    },
    "OCR": {
        "Screenshot as an API": "  - Enterprise-grade [[**OCR**]] for [[PDF"
    },
    "Prompt Engineering": {
        "Screenshot as an API": " - (encoder) diffusion-based models like Stable Diffusion and DALL-E (text-to-image) [[**Prompt Engineering**]]\n",
        "Metaverse": "to my mind that Metaverse can be attained within years with [[**Prompt Engineering**]].\n",
        "2022-09-03": "- [[**Prompt Engineering**]]\n",
        "2022-09-30": "  - [[**Prompt Engineering**]]\n"
    },
    "PDF": {
        "Screenshot as an API": "  - Enterprise-grade OCR for [[**PDF**]] documents still doesn't exist in 2022, albeit having state-of-the-art computer vision",
        "2022-06-05": "- _Convert ACSM files to [[**PDF**]]/EPUBs with one command on Linux \\(and macOS very soon\\). This software"
    },
    "Naver": {
        "Screenshot as an API": "  - Related to when [[**Naver**]] Vibe attempted to steal other music players' market cap with Screenshot"
    },
    "IRS": {
        "Screenshot as an API": "- The \"killer app\" for PDF was tax returns - the [[**IRS**]] adopted PDF in 1996 because of a rumored frustration with the"
    },
    "epub": {
        "Screenshot as an API": "  - Slow page loads. Better alternatives: [[**epub**]], [[mobi"
    },
    "mobi": {
        "Screenshot as an API": "  - Slow page loads. Better alternatives: epub, [[**mobi**]] for texts. For generic use cases, [[DjVu"
    },
    "DjVu": {
        "Screenshot as an API": "loads. Better alternatives: epub, mobi for texts. For generic use cases, [[**DjVu**]].\n"
    },
    "Transactional Globe": {
        "3D Globe on the Web": "- [[**Transactional Globe**]]\n",
        "Better Aldehyde": "- [[**Transactional Globe**]]\n",
        "2022-10-10": "- [[**Transactional Globe**]]\n"
    },
    "GDPR": {
        "Simple Analytics": "- [[**GDPR**]], [[PECR",
        "Plausible Analytics": "lightweight and open-source web analytics. No cookies and fully compliant with [[**GDPR**]], [[CCPA"
    },
    "PECR": {
        "Simple Analytics": "- GDPR, [[**PECR**]], [[CCPA",
        "Plausible Analytics": "web analytics. No cookies and fully compliant with GDPR, CCPA, and [[**PECR**]]. Made and hosted in the EU, powered by European-owned cloud infrastructure"
    },
    "CCPA": {
        "Simple Analytics": "- GDPR, PECR, [[**CCPA**]] Compliant\n",
        "Plausible Analytics": "and open-source web analytics. No cookies and fully compliant with GDPR, [[**CCPA**]], and [[PECR"
    },
    "2022-07-01": {
        "PKM": "- On [[**2022-07-01**]], I noted that PKM is for building a personal knowledge base."
    },
    "PKM": {
        "PKM": "| [[**PKM**]]        | Architecture for Digital"
    },
    "PARA": {
        "PKM": "| [[**PARA**]]       | Architecture for Second Brain.",
        "CODE": "- Often used with [[**PARA**]].\n",
        "2022-07-01": "I like the CODE Procedure in [[**PARA**]] architecture as a project management system, which does more than [[PKM",
        "2022-06-26": "- [[**PARA**]]\n",
        "2022-07-03": "- Partially implemented [[**PARA**]] on this brain.\n",
        "2022-07-06": "- [[**PARA**]]... might be worse than I thought?\n"
    },
    "Evergreen Notes": {
        "PKM": "grow into a creative garage | Focuses on creating and tending [[**Evergreen Notes**]] posts that grow into a knowledge base |\n",
        "Transient Notes": "- Antonym: [[**Evergreen Notes**]]\n",
        "2022-07-01": "grow into a creative garage | Focuses on creating and tending [[**Evergreen Notes**]] posts that grow into a knowledge base |\n",
        "2022-05-29": "- [[**Evergreen Notes**]] vs [[Transient Notes"
    },
    "MessagePack": {
        "CBOR": "- Compact Data Exchange Format like [[**MessagePack**]].\n",
        "2022-07-11": "Candidates include [[**MessagePack**]] and [[CBOR"
    },
    "Our tools also shape us": {
        "We become what we do": "- [[**Our tools also shape us**]]\n",
        "2022-10-13": "- [[**Our tools also shape us**]]\n"
    },
    "social network": {
        "Jumpsite": "or Web Linking Network Service, is a collective term for a [[**social network**]] service mainly consisting of links to outside content.\n"
    },
    "Hacker News": {
        "Jumpsite": "## [[**Hacker News**]]\n",
        "Better Hacker News": "## [[**Hacker News**]] need some improvements\n",
        "2022-07-05": "## Today's [[**Hacker News**]]\n",
        "2022-07-20": "## Today's [[**Hacker News**]]\n",
        "2022-10-13": "## [[**Hacker News**|HN]]\n",
        "2022-06-21": "## Today's [[**Hacker News**]]\n",
        "2022-09-29": "## Today's [[**Hacker News**]]\n",
        "2022-09-09": "## Today's [[**Hacker News**]]\n",
        "2022-07-09": "wikilinks to markdown links. For example, this is a wikilink to [[**Hacker News**]].\n",
        "2022-06-22": "## Today's [[**Hacker News**]]\n",
        "2022-07-18": "## Today's [[**Hacker News**]]\n",
        "2022-08-01": "## Today's [[**Hacker News**]]\n",
        "2022-09-30": "## Today's [[**Hacker News**]]\n",
        "2022-09-10": "## Today's [[**Hacker News**]]\n",
        "2022-10-10": "## Today's [[**Hacker News**]]\n",
        "2022-07-26": "## Today's [[**Hacker News**]]\n",
        "2022-09-05": "- An excellent recommendation from the original discussion on [[**Hacker News**|HN]] was to use [ImprovMX](https://improvmx.com) as an outgoing email server.\n",
        "2022-10-15": "## [[**Hacker News**|HN]]\n"
    },
    "Pasted image 20221013004141.png": {
        "Jumpsite": "![[**Pasted image 20221013004141.png**]]\n",
        "Hacker News": "![[**Pasted image 20221013004141.png**]]\n"
    },
    "Daily Dev": {
        "Jumpsite": "## [[**Daily Dev**]]\n"
    },
    "Pasted image 20221013004218.png": {
        "Jumpsite": "![[**Pasted image 20221013004218.png**]]\n",
        "Daily Dev": "![[**Pasted image 20221013004218.png**]]\n"
    },
    "Surfit": {
        "Jumpsite": "## [[**Surfit**]]\n"
    },
    "Are.na": {
        "Jumpsite": "## [[**Are.na**]]\n"
    },
    "Mood.surf": {
        "Jumpsite": "## [[**Mood.surf**]]\n"
    },
    "Pasted image 20221013005758.png": {
        "Jumpsite": "![[**Pasted image 20221013005758.png**]]\n",
        "Mood.surf": "![[**Pasted image 20221013005758.png**]]\n"
    },
    "Pasted image 20220801181647.png": {
        "Pretendard의 아쉬운 점": "![[**Pasted image 20220801181647.png**]]\n"
    },
    "CJK": {
        "Pretendard의 아쉬운 점": "일반적으로 알려지지 않은 사항이지만, [[**CJK**|한중일]]의 한문은 모두 조금씩 다르다.\n",
        "Stories Behind Satori": "added Google Font's `?text=` API, containing a specific subset of the [[**CJK**]] library and rendering things on the fly.\n"
    },
    "9E21DD.gif": {
        "Pretendard의 아쉬운 점": "![[**9E21DD.gif**]]\n"
    },
    "고통에는 순응이 아니라 감작이 나타난다": {
        "월급만큼 중요한 것은 자존급이다": "게다가 [[**고통에는 순응이 아니라 감작이 나타난다**]].\n"
    },
    "Financial Technology": {
        "Toss": "- [[**Financial Technology**]] Unicorn of Korea.\n"
    },
    "Tossface": {
        "Toss": "- Creators of Toss Product Sans and [[**Tossface**]]\n"
    },
    "Transient Notes": {
        "Evergreen Notes": "- Antonym: [[**Transient Notes**]]\n",
        "2022-05-29": "- Evergreen Notes vs [[**Transient Notes**]]\n"
    },
    "Markdown": {
        "Inliner": "However, [[**Markdown**]] will automatically convert them into flowing text, making the reading experience",
        "Markdown Email Client": "- You write an email in [[**Markdown**]]\n"
    },
    "Pasted image 20220917153155.png": {
        "Android Equivalent of Div": "![[**Pasted image 20220917153155.png**]]\n"
    },
    "Visual Studio Code": {
        "Jetbrains Fleet": "- a potential alternative to [[**Visual Studio Code**]]\n"
    },
    "Game Programming": {
        "Back Buffer": "Used in [[**Game Programming**]], you write data on the back buffer.\n"
    },
    "Google": {
        "WorkerDOM": "[[**Google**]]'s take on providing DOM access to [[Web Worker",
        "Stories Behind Satori": "They also added [[**Google**]] Font's `?text=` API, containing a specific subset of the [[CJK",
        "2022-07-21": "### [[**Google**]], really? Come on.\n",
        "2022-07-14": "more. One wrong info that I found: Uber didn't make Kubernetes. [[**Google**]] did.\n",
        "2022-06-22": "[[**Google**]]'s [[WorkerDOM"
    },
    "Brane": {
        "WorkerDOM": "[[**Brane**]] will follow WorkerDOM's core model.\n",
        "iiframe": "- Successor to [[**Brane**]].\n",
        "2022-07-11": "To automatically use Atomics API for every third-party app, [[**Brane**]] should **_abstract_** this process.\n",
        "2022-07-05": "business. I do PoCs that have never been done before, i.e., [[**Brane**]]\n",
        "2022-08-03": "[[**Brane**]] is a vast project that a single intern can't finish.\n",
        "2022-06-14": "- [[**Brane**]] DOM의 재구현이 의미가 있는지 실험하고, 유의미한 전략인지 확인하는 것이 목표이다.\n",
        "2022-06-22": "Since [[**Brane**]] implements an _arbiter_, which spans more than just a DOM...\n",
        "2022-06-26": "## [[**Brane**]] Notes\n",
        "2022-07-12": "[[**Brane**]]'s previous demo goals were:\n",
        "2022-07-06": "### [[**Brane**]] [[WorkerDOM"
    },
    "SharedArrayBuffer": {
        "WorkerDOM": "WorkerDOM doesn't use [[**SharedArrayBuffer**]], most likely to Spectre.\n",
        "Atomics": "- [[**SharedArrayBuffer**]]를 이용해 [[Web Worker",
        "Providing DOM API to Worker Threads": "- `heart` starts beating, incrementing [[**SharedArrayBuffer**|SAB]] `[0]`.\n",
        "2022-07-25": "- [ ] Pass the data into the received [[**SharedArrayBuffer**]].\n",
        "2022-07-11": "We will need a codec to share data on [[**SharedArrayBuffer**]].\n",
        "2022-07-21": "  - Pass the data into the received [[**SharedArrayBuffer**]].\n",
        "2022-08-03": "This happens synchronously using Atomics and [[**SharedArrayBuffer**]].\n",
        "2022-06-27": "- Learn more about Atomics and [[**SharedArrayBuffer**]]\n",
        "2022-07-19": "  - 받은 [[**SharedArrayBuffer**|SAB]]에 데이터를 채운다\n",
        "2022-06-22": "  - WorkerDOM will harness [[**SharedArrayBuffer**]].\n",
        "2022-06-26": "- **Objective**: Swap WorkerDOM's data transfer layer with [[**SharedArrayBuffer**]] + Shim Core DOM APIs.\n",
        "2022-07-12": "   - This involves [[**SharedArrayBuffer**|SAB]]\n"
    },
    "Web Typography": {
        "San Francisco": "- Has a strict usage model. No [[**Web Typography**|Webfont]] use\n"
    },
    "Brunch violates Apple Terms with SF": {
        "San Francisco": "- Although [[**Brunch violates Apple Terms with SF**]]\n"
    },
    "Extracranial": {
        "Research": "- [[**Extracranial**]]\n",
        "2022-10-13": "- [[**Extracranial**]]\n",
        "2022-09-28": "- [[**Extracranial**]]\n",
        "2022-06-14": "- [[**Extracranial**]]\n",
        "2022-05-28": "- [[**Extracranial**]]\n",
        "2022-06-26": "- [[**Extracranial**]].\n",
        "2022-09-30": "- [Chris Welch](https://chriswelch.co/) [[**Extracranial**]]\n",
        "2022-05-27": "- [[**Extracranial**]]\n",
        "2022-09-25": "- [[**Extracranial**]]\n",
        "2022-07-02": "### [[**Extracranial**]]\n"
    },
    "Preemptive Multitasking": {
        "Cooperative Multitasking": "Antonym of [[**Preemptive Multitasking**]].\n"
    },
    "Person A480C6": {
        "Apple Silicon": "- [[**Person A480C6**]]\n",
        "Project PEOPLE": "- [[**Person A480C6**]]\n"
    },
    "Code Quality": {
        "Game Programming": "- [[**Code Quality**]]\n",
        "2022-09-12": "- [[**Code Quality**]]\n"
    },
    "Delta Time": {
        "Game Programming": "- [[**Delta Time**]]\n",
        "Graphics Engine Process": "    - If [[**Delta Time**]] is bigger than Max [[**Delta Time**"
    },
    "Graphics Engine Process": {
        "Game Programming": "- [[**Graphics Engine Process**]]\n"
    },
    "Forward Declaration": {
        "Game Programming": "- [[**Forward Declaration**]]\n"
    },
    "Simple DirectMedia Layer": {
        "Game Programming": "- [[**Simple DirectMedia Layer**]]\n"
    },
    "Cooperative Multitasking": {
        "Preemptive Multitasking": "Antonym of [[**Cooperative Multitasking**]].\n"
    },
    "2022-07-14": {
        "Fiercely Overpriced": "An idea suggested on [[**2022-07-14**]] that all inventions should be _fiercely overpriced_.\n",
        "0125 Valid Palindrome": "Solved at: [[**2022-07-14**]] and [[2022-07-26"
    },
    "Metadata as a Service": {
        "Fiercely Overpriced": "[[**Metadata as a Service**]] could also be one example.\n"
    },
    "2022-07-24": {
        "FIRE Engine": "[How Much Money Do Indie Hackers Products Make? | Scraping Fish](https://scrapingfish.com/blog/indie-hackers-revenue) ([[**2022-07-24**]])\n"
    },
    "TODO": {
        "Tools for Thought": "[[**TODO**]]: Convert this page into a more in-depth case study.\n",
        "Coding Tests": "- 0215 Kth Largest Element in an Array [[**TODO**]]\n",
        "0215 Kth Largest Element in an Array": "[[**TODO**]]\n",
        "2022-10-12": "## [[**TODO**]]\n",
        "2022-10-16": "## [[**TODO**]]\n",
        "2022-10-13": "## [[**TODO**]]\n"
    },
    "Notion": {
        "Tools for Thought": "- [Craft](https://www.craft.do/): [[**Notion**]]-like service, but more focus on supporting native platforms.\n",
        "2022-07-10": "- [[**Notion**]] opened a new paradigm for low-code websites. [[Project PEOPLE",
        "2022-07-07": "I think even [[**Notion**]] is too complicated for personal task managing.\n"
    },
    "JavaScript": {
        "Tools for Thought": "- [Bear Blog](https://bearblog.dev/): No trackers, no [[**JavaScript**|javascript]], no stylesheets blog for hackers\n",
        "Deno Deploy": "- Deploy [[**JavaScript**|**JavaScript**]] Globally\n",
        "TypeScript": "- Typed [[**JavaScript**]].\n",
        "Separation of Computer Scientists and Computer Technologists": "A self-taught dev can use React to make a cool-looking Single-Page-App [[**JavaScript**]] e-commerce website...\n",
        "2022-07-11": "Blocking in [[**JavaScript**]] can be done in two ways.\n",
        "2022-07-01": "- This will reduce 10kb in [[**JavaScript**|**JavaScript**]].\n",
        "2022-07-20": "- [[**JavaScript**|**JavaScript**]] → [[TypeScript|TypeScript",
        "2022-06-20": "are increasingly API-driven. CAPTCHAs cannot work in an API environment where [[**JavaScript**|**JavaScript**]] can't be rendered or a WebView can't be called.\n",
        "2022-06-22": "Then, Brane should cover very partial [[**JavaScript**]] APIs.\n",
        "2022-07-18": "- No trackers, no [[**JavaScript**|javascript]], no stylesheets. Just your words\n",
        "2022-06-12": "web development, hydration or rehydration is a technique in which client-side [[**JavaScript**|**JavaScript**]] converts a static [[HTML",
        "2022-07-12": "1. Demo [[**JavaScript**]] animation works as expected.\n",
        "2022-07-06": "- Bundle, transpile, install and run [[**JavaScript**|**JavaScript**]] & [[TypeScript|TypeScript"
    },
    "NextJS": {
        "Tools for Thought": "- [[**NextJS**]]\n",
        "2022-09-30": "### Vercel [[**NextJS**]] Conf Interactions\n"
    },
    "Graphic Window": {
        "Graphics Engine Process": "  - Create a [[**Graphic Window**]]\n"
    },
    "Graphic Renderer": {
        "Graphics Engine Process": "  - Create a [[**Graphic Renderer**]]\n"
    },
    "FPS": {
        "Graphics Engine Process": "  - Limit Updating Frequency to 16ms, which yields 60 [[**FPS**]].\n",
        "2022-10-10": "23.976 and not 24 fps?? - AC's & DIT's - Cinematography.com](https://cinematography.com/index.php?/forums/topic/71346-why-23976-and-not-24-fps/&tab=comments#comment-455454). [[**FPS**]].\n"
    },
    "Back Buffer": {
        "Graphics Engine Process": "    - Present the [[**Back Buffer**]]\n"
    },
    "Slow down your brain clock cycle": {
        "Handwriting Tools for Thoughts": "  - Benefit: handwritings [[**Slow down your brain clock cycle**]]\n",
        "2022-10-12": "- [[**Slow down your brain clock cycle**]]\n",
        "2022-10-13": "- [[**Slow down your brain clock cycle**]]\n"
    },
    "Handwriting Grammarly": {
        "Handwriting Tools for Thoughts": "- We might need [[**Handwriting Grammarly**]]\n"
    },
    "Better Hacker News": {
        "Handwriting Tools for Thoughts": "- [[**Better Hacker News**]]\n",
        "2022-10-12": "- [[**Better Hacker News**]]\n",
        "2022-07-12": "- [[**Better Hacker News**]]\n"
    },
    "418938.jpeg": {
        "Handwriting Tools for Thoughts": "![[**418938.jpeg**]]\n",
        "Better Hacker News": "![[**418938.jpeg**]]\n",
        "2022-10-12": "![[**418938.jpeg**]]\n"
    },
    "Font Features": {
        "Toss Product Sans": "- I figured it did not support [[**Font Features**]].\n",
        "CalliFontia": "- 추출 전 여러가지 [[**Font Features**|Font Feature]]을 켜고 끄며 미리보기 대조 가능\n",
        "2022-08-01": "- Just realized that this enables Font [[**Font Features**|Ligature]] Settings.\n"
    },
    "Shim": {
        "Realm Shim": "> [[**Shim**]] implementation of the [Realm API Proposal](https://github.com/tc39/proposal-realms/#ecmascript-spec-proposal-for-realms-api) ([[ShadowRealm"
    },
    "ShadowRealm": {
        "Realm Shim": "> Shim implementation of the [Realm API Proposal](https://github.com/tc39/proposal-realms/#ecmascript-spec-proposal-for-realms-api) ([[**ShadowRealm**]])\n",
        "2022-06-26": "- Let's not explore [[**ShadowRealm**]] and `iframe` [[Sandbox"
    },
    "Grammarly": {
        "Handwriting Grammarly": "- [[**Grammarly**]] for [[Handwriting Tools for Thoughts",
        "2022-10-12": "- [x] Reply to [[**Grammarly**]] Recruiter\n"
    },
    "Handwriting Tools for Thoughts": {
        "Handwriting Grammarly": "- Grammarly for [[**Handwriting Tools for Thoughts**]]\n",
        "2022-10-12": "- [[**Handwriting Tools for Thoughts**]]\n"
    },
    "Building a Second Brain": {
        "PARA": "- PARA is a second-brain framework, mentioned in [[**Building a Second Brain**]]\n",
        "CODE": "- The CODE Procedure is a second-brain building procedure introduced in [[**Building a Second Brain**]].\n",
        "2022-07-01": "## [[**Building a Second Brain**]]\n",
        "2022-06-14": "- _[[**Building a Second Brain**]]: A Proven Method to Organize Your Digital Life and Unlock Your",
        "Readings": "- [x] [[**Building a Second Brain**]]\n"
    },
    "CODE": {
        "PARA": "- Often used with [[**CODE**]]\n"
    },
    "Pasted image 20220913001354.png": {
        "Ignoring $ for copy-pasting online resources to terminal": "![[**Pasted image 20220913001354.png**]]\n"
    },
    "Metaverse": {
        "Prompt Engineering": "- It has now come to my mind that [[**Metaverse**]] can actually be attained within years with Prompt Engineering.\n",
        "2022-05-31": "- Kakao announced [[**Metaverse**]] Remote Work Policy.\n"
    },
    "Personal Training Corpus": {
        "Prompt Engineering": "- [[**Personal Training Corpus**]]\n",
        "2022-10-12": "- [[**Personal Training Corpus**]]\n"
    },
    "Height": {
        "Linear": "- An issue tracking service, like [[**Height**]], or [[Jira"
    },
    "Jira": {
        "Linear": "- An issue tracking service, like Height, or [[**Jira**]]\n"
    },
    "CBOR": {
        "MessagePack": "- Compact Data Exchange Format like [[**CBOR**]].\n",
        "2022-07-11": "Candidates include MessagePack and [[**CBOR**]].\n"
    },
    "Canvas (HTML5)": {
        "Stories Behind Satori": "- [[**Canvas (HTML5)**|Canvas]]-based solutions lacked dynamic behaviors (e.g., layout) and APIs.\n"
    },
    "Web Browser": {
        "Stories Behind Satori": "- [[**Web Browser**]]-based solutions were too slow\n"
    },
    "JSX": {
        "Stories Behind Satori": "After simplifying the syntax with [[**JSX**]] and simple [[CSS"
    },
    "PNG": {
        "Stories Behind Satori": "Shu used [RazrFalcon/resvg](https://github.com/RazrFalcon/resvg) and [yisibl/resvg-js](https://github.com/yisibl/resvg-js) to convert the SVG image to [[**PNG**]].\n"
    },
    "Dogfooding": {
        "Stories Behind Satori": "The Vercel team [[**Dogfooding**|dogfooded]] this for a while.\n"
    },
    "Pure Component Model": {
        "Monolithic (Pure Hierarchy) Model": "- Solution [[**Pure Component Model**]]\n",
        "2022-09-03": "- [[**Pure Component Model**]]\n"
    },
    "Pasted image 20221014171925.png": {
        "Linear README": "![[**Pasted image 20221014171925.png**]]\n"
    },
    "FIRE Engine": {
        "FIRE": "- [[**FIRE Engine**]]\n",
        "Old Fashioned Camera": "- 백엔드와 프론트가 모두 필요 없기에 [[**FIRE Engine**]]을 실행하기에 적합해보임\n",
        "CELLO (Product)": "- [[**FIRE Engine**]]\n",
        "Cube": "- [[**FIRE Engine**]]\n",
        "Intracranial": "- [[**FIRE Engine**]]\n",
        "2022-07-24": "- [[**FIRE Engine**]]\n",
        "2022-10-13": "- [[**FIRE Engine**]]\n",
        "2022-07-18": "- [[**FIRE Engine**]]\n",
        "2022-07-26": "PencilKit](https://github.com/simonbs/InfiniteCanvas), it seems relatively easy to convert this into a full-blown [[**FIRE Engine**]].\n"
    },
    "Project PEOPLE": {
        "Separation of Computer Scientists and Computer Technologists": "- [[**Project PEOPLE**]] in the academic fields support those works.\n",
        "Person A480C6": "  - [[**Project PEOPLE**]] can take responsibility, regardless of their level and position.\n",
        "Person 5DBDAE": "Meet a lot of [[**Project PEOPLE**]].\n",
        "2022-07-05": "- Some [[**Project PEOPLE**]] asked why it is _computer scientist_ instead of _software engineer_... And",
        "2022-07-10": "- Notion opened a new paradigm for low-code websites. [[**Project PEOPLE**]] now create personal websites, event RSVPs, blogs, databases, and much more,",
        "2022-09-08": "- [[**Project PEOPLE**]]\n",
        "2022-09-14": "- [[**Project PEOPLE**]]\n"
    },
    "Fiercely Overpriced": {
        "CalliFontia": "## [[**Fiercely Overpriced**]]\n",
        "Off The Record": "- [[**Fiercely Overpriced**]]: disappears after opening. max 10 seconds (or message length)\n",
        "CELLO (Product)": "## How would it be [[**Fiercely Overpriced**]]?\n",
        "Open Graph Image as a Service": "## How would it be [[**Fiercely Overpriced**]]?\n",
        "2022-07-14": "- Moved to [[**Fiercely Overpriced**]]\n",
        "2022-07-18": "- [[**Fiercely Overpriced**]] for [[Open Graph Image as a Service"
    },
    "Pasted image 20221003145256.png": {
        "sem.sh": "![[**Pasted image 20221003145256.png**]]\n"
    },
    "Pasted image 20221003150852.png": {
        "sem.sh": "  - ![[**Pasted image 20221003150852.png**]]\n"
    },
    "Pasted image 20221003151628.png": {
        "sem.sh": "  - ![[**Pasted image 20221003151628.png**]]\n"
    },
    "Pasted image 20221003151639.png": {
        "sem.sh": "  - ![[**Pasted image 20221003151639.png**]]\n"
    },
    "Pasted image 20221003151648.png": {
        "sem.sh": "  - ![[**Pasted image 20221003151648.png**]]\n"
    },
    "Dynamic Island": {
        "Dynamic Island on the Web": "- [[**Dynamic Island**]] on the Web\n",
        "2022-09-30": "### [[**Dynamic Island**]] Concepts for Desktops\n"
    },
    "0C4D2A.jpg": {
        "Dynamic Island on the Web": "![[**0C4D2A.jpg**]]\n",
        "2022-10-15": "![[**0C4D2A.jpg**]]\n"
    },
    "Framer Motion": {
        "Dynamic Island on the Web": "- [[**Framer Motion**]] seems appropriate.\n"
    },
    "7F65A5.jpeg": {
        "Dynamic Island on the Web": "![[**7F65A5.jpeg**]]\n"
    },
    "CELLO": {
        "ULLO": "- Unsplash [[**CELLO**]]\n",
        "CELLO (Product)": "그것의 Consumer-side Product가 [[**CELLO**]]\n",
        "2022-08-10": "- [[**CELLO**]]\n"
    },
    "Cloudflare": {
        "CELLO (Product)": "- [[**Cloudflare**]] for serving high-res images\n",
        "2022-08-28": "- **Key Strengths of Vercel over [[**Cloudflare**]] Pages**. Vercel builds faster (~3m) than [[**Cloudflare**",
        "2022-06-20": "[[**Cloudflare**]]: [Private Access Tokens: eliminating CAPTCHAs on iPhones and Macs with open"
    },
    "Deno Deploy": {
        "CELLO (Product)": "- Possibly [[**Deno Deploy**]]\n"
    },
    "K-Pop Apps": {
        "CELLO (Product)": "- Avoid local confinement, as seen in most [[**K-Pop Apps**]].\n"
    },
    "CELLO (Product)": {
        "CELLO": "- [[**CELLO (Product)**]]\n",
        "2022-08-04": "### [[**CELLO (Product)**]]\n",
        "2022-07-18": "- [[**CELLO (Product)**]]\n"
    },
    "ULLO": {
        "CELLO": "- [[**ULLO**]]\n",
        "2022-08-04": "- Intermediary PoC Product: [[**ULLO**]]\n"
    },
    "퍼블리시티권": {
        "CELLO": "- [[**퍼블리시티권**]]\n"
    },
    "Python": {
        "get-a-job-done": "  - Building [[**Python**]] Data Parser\n",
        "0001 Two Sum": "I used [[**Python**]] Dictionary to store complementing values. [[**Python**",
        "Coding Tests": "- Will use [[**Python**]] (main language) and [[C++",
        "Python": "You can offset with [[**Python**]]'s `enumerate` function with list splitting.\n"
    },
    "2022-09-25": {
        "0110 Balanced Binary Tree": "Solved at: [[**2022-09-25**]]\n",
        "0235 LCA of a Binary Search Tree": "Solved at: [[**2022-09-25**]]. Took **17m 09s**\n",
        "1290 Convert Binary Number in a Linked List to Integer": "Solved at: [[**2022-09-25**]]\n"
    },
    "2022-07-10": {
        "0001 Two Sum": "Solved at: [[**2022-07-10**]]\n",
        "0020 Valid Parentheses": "Solved at: [[**2022-07-10**]]\n"
    },
    "Coding Tests": {
        "Leetcode": "- [[**Coding Tests**]]\n",
        "2022-09-26": "## [[**Coding Tests**]]\n",
        "2022-09-22": "## [[**Coding Tests**]]\n",
        "2022-09-07": "## [[**Coding Tests**]]\n",
        "2022-07-10": "## [[**Coding Tests**]]\n",
        "2022-08-26": "- [[**Coding Tests**]]\n",
        "2022-08-27": "## [[**Coding Tests**]]\n",
        "2022-08-30": "## [[**Coding Tests**]]\n",
        "2022-06-06": "- _[[**Coding Tests**]] questions_ - [Grind 75 - A better Blind 75 you can",
        "2022-09-20": "## [[**Coding Tests**]]\n",
        "2022-09-30": "## [[**Coding Tests**]]\n",
        "2022-09-04": "## [[**Coding Tests**]]\n",
        "2022-09-24": "## [[**Coding Tests**]]\n",
        "2022-07-13": "## [[**Coding Tests**]]\n",
        "2022-09-10": "## [[**Coding Tests**]]\n",
        "2022-09-25": "## [[**Coding Tests**]]\n",
        "2022-07-26": "## [[**Coding Tests**]]\n",
        "2022-09-11": "## [[**Coding Tests**]]\n",
        "2022-07-02": "- Let's start from [[**Coding Tests**]] starting next week.\n",
        "2022-09-21": "## [[**Coding Tests**]]\n",
        "2022-09-05": "## [[**Coding Tests**]]\n",
        "2022-10-15": "## [[**Coding Tests**]]\n"
    },
    "2022-09-04": {
        "0242 Valid Anagram": "Solved at: [[**2022-09-04**]]\n",
        "0704 Binary Search": "Solved at: [[**2022-09-04**]]\n"
    },
    "Real Exams": {
        "Coding Tests": "- [[**Real Exams**]]\n",
        "2022-09-26": "- [[**Real Exams**]]\n",
        "2022-10-12": "  - [x] [[**Real Exams**]]\n",
        "2022-09-07": "- [[**Real Exams**]]\n",
        "2022-09-24": "- [[**Real Exams**]]\n",
        "2022-09-10": "- [[**Real Exams**]]\n",
        "2022-09-11": "- [[**Real Exams**]]\n",
        "2022-09-21": "- [[**Real Exams**]]\n",
        "2022-09-05": "- [[**Real Exams**]]\n"
    },
    "Real Interviews": {
        "Coding Tests": "- [[**Real Interviews**]]\n",
        "2022-09-22": "- [[**Real Interviews**]]\n",
        "2022-09-30": "- [[**Real Interviews**]]\n"
    },
    "C++": {
        "Coding Tests": "- Will use Python (main language) and [[**C++**]] (supplementary language)\n"
    },
    "QuickSelect": {
        "Coding Tests": "- [[**QuickSelect**]] [[TODO",
        "0215 Kth Largest Element in an Array": "- Use [[**QuickSelect**]]\n"
    },
    "Trie": {
        "Coding Tests": "- [[**Trie**]] [[TODO"
    },
    "1448 Count Good Nodes in Binary Tree": {
        "Coding Tests": "- [[**1448 Count Good Nodes in Binary Tree**]]\n"
    },
    "0002 Add Two Numbers": {
        "Coding Tests": "- [[**0002 Add Two Numbers**]]\n"
    },
    "0199 Binary Tree Right Side View": {
        "Coding Tests": "- [[**0199 Binary Tree Right Side View**]]\n"
    },
    "1046 Last Stone Weight": {
        "Coding Tests": "- [[**1046 Last Stone Weight**]]\n",
        "2022-08-30": "- [[**1046 Last Stone Weight**]]\n"
    },
    "0215 Kth Largest Element in an Array": {
        "Coding Tests": "- [[**0215 Kth Largest Element in an Array**]] [[TODO"
    },
    "0079 Word Search": {
        "Coding Tests": "- [ ] [[**0079 Word Search**]]\n"
    },
    "1290 Convert Binary Number in a Linked List to Integer": {
        "Coding Tests": "- [[**1290 Convert Binary Number in a Linked List to Integer**]]\n",
        "2022-09-25": "- [[**1290 Convert Binary Number in a Linked List to Integer**]]\n"
    },
    "0001 Two Sum": {
        "Coding Tests": "- [x] [[**0001 Two Sum**]]\n",
        "2022-07-10": "- Solved [[**0001 Two Sum**]]\n"
    },
    "0020 Valid Parentheses": {
        "Coding Tests": "- [x] [[**0020 Valid Parentheses**]]\n",
        "2022-07-10": "- Solved [[**0020 Valid Parentheses**]]\n"
    },
    "0021 Merge Two Sorted Lists": {
        "Coding Tests": "- [x] [[**0021 Merge Two Sorted Lists**]]\n",
        "2022-07-13": "- Solved [[**0021 Merge Two Sorted Lists**]]\n"
    },
    "0121 Best Time to Buy and Sell Stock": {
        "Coding Tests": "- [x] [[**0121 Best Time to Buy and Sell Stock**]]\n",
        "2022-07-13": "- Solved [[**0121 Best Time to Buy and Sell Stock**]]\n"
    },
    "0125 Valid Palindrome": {
        "Coding Tests": "- [x] [[**0125 Valid Palindrome**]]\n",
        "2022-07-26": "- [x] [[**0125 Valid Palindrome**]]\n"
    },
    "0226 Invert Binary Tree": {
        "Coding Tests": "- [x] [[**0226 Invert Binary Tree**]]\n",
        "2022-07-26": "- [x] [[**0226 Invert Binary Tree**]]\n"
    },
    "0242 Valid Anagram": {
        "Coding Tests": "- [x] [[**0242 Valid Anagram**]]\n",
        "2022-09-04": "- [[**0242 Valid Anagram**]]\n"
    },
    "0704 Binary Search": {
        "Coding Tests": "- [x] [[**0704 Binary Search**]]\n",
        "2022-09-05": "- [[**0704 Binary Search**]]\n"
    },
    "0733 Flood Fill": {
        "Coding Tests": "- [x] [[**0733 Flood Fill**]]\n",
        "2022-09-05": "- [[**0733 Flood Fill**]]\n"
    },
    "0053 Maximum Subarray": {
        "Coding Tests": "- [x] [[**0053 Maximum Subarray**]]\n",
        "2022-09-05": "- [[**0053 Maximum Subarray**]]\n"
    },
    "0235 LCA of a Binary Search Tree": {
        "Coding Tests": "- [x] [[**0235 LCA of a Binary Search Tree**]]\n",
        "2022-09-25": "- [[**0235 LCA of a Binary Search Tree**]]\n"
    },
    "0110 Balanced Binary Tree": {
        "Coding Tests": "- [x] [[**0110 Balanced Binary Tree**]]\n",
        "2022-09-25": "- [[**0110 Balanced Binary Tree**]]\n"
    },
    "Pasted image 20220904230841.png": {
        "Python": "![[**Pasted image 20220904230841.png**]]\n"
    },
    "2022-07-26": {
        "0226 Invert Binary Tree": "Solved at: [[**2022-07-26**]]\n",
        "0125 Valid Palindrome": "Solved at: 2022-07-14 and [[**2022-07-26**]]\n",
        "Intracranial": "- [[**2022-07-26**]]\n",
        "Developing Review Notes for Obsidian": "## [[**2022-07-26**]]\n"
    },
    "2022-08-30": {
        "1046 Last Stone Weight": "Solved at: [[**2022-08-30**]]\n",
        "0215 Kth Largest Element in an Array": "Solved at: [[**2022-08-30**]]\n"
    },
    "2022-08-27": {
        "1448 Count Good Nodes in Binary Tree": "Solved at: [[**2022-08-27**]]\n",
        "0002 Add Two Numbers": "Solved at: [[**2022-08-27**]]\n",
        "0199 Binary Tree Right Side View": "Solved at: [[**2022-08-27**]]\n"
    },
    "binary search": {
        "0704 Binary Search": "- [[**binary search**]]\n",
        "Real Exams": "- 1 binary tree construction & [[**binary search**]] traversal\n",
        "2022-09-05": "- [[**binary search**]]\n"
    },
    "simulation": {
        "optimized simulation": "Optimized Simulation programming problem is when you have to conduct a [[**simulation**]] according to the provided conditions.\n",
        "Real Exams": "- 2 [[**simulation**]]s\n",
        "mathematic coding": "  - conducting a bare metal [[**simulation**]] takes too long\n"
    },
    "2022-09-05": {
        "0733 Flood Fill": "Solved at: [[**2022-09-05**]]\n",
        "0053 Maximum Subarray": "Solved at: [[**2022-09-05**]]\n"
    },
    "0 for i in image[0": {
        "0733 Flood Fill": "        visited = [[**0 for i in image[0**]] for j in image]\n"
    },
    "BFS": {
        "0733 Flood Fill": "I did [[**BFS**]], but it seems possible to do this in recursion too.\n",
        "Real Exams": "- [[**BFS**]]\n",
        "Real Interviews": "    - 1 conditional [[**BFS**]] problem\n"
    },
    "optimized simulation": {
        "Real Exams": "- 1 [[**optimized simulation**]]\n",
        "simulation": "- If there are time-space restrictions, look into [[**optimized simulation**]]s.\n",
        "2022-09-05": "- [[**optimized simulation**]]\n"
    },
    "mathematic coding": {
        "Real Exams": "- 1 [[**mathematic coding**]]\n",
        "2022-09-05": "- [[**mathematic coding**]]\n"
    },
    "optimized simulations": {
        "Real Exams": "- 2 [[**optimized simulations**]]\n"
    },
    "greedy": {
        "Real Exams": "- 1 [[**greedy**]] palindrome substring search\n"
    },
    "get-a-job-done": {
        "Real Exams": "- [[**get-a-job-done**]] Data Parsing\n",
        "2022-09-05": "- [[**get-a-job-done**]]\n"
    },
    "2022-07-13": {
        "0021 Merge Two Sorted Lists": "Solved at: [[**2022-07-13**]]\n",
        "0121 Best Time to Buy and Sell Stock": "Solved at: [[**2022-07-13**]]\n"
    },
    "Leetcode": {
        "Real Interviews": "  - 1 [[**Leetcode**]] Hard level question\n"
    },
    "2022-08-26": {
        "0079 Word Search": "Solved at: [[**2022-08-26**]]\n"
    },
    "Backlinking": {
        "Better Aldehyde": "- [[**Backlinking**]]\n"
    },
    "Backlink": {
        "Backlinking": "- [[**Backlink**]]\n",
        "Better Docusaurus": "- Docusaurus should have [[**Backlink**]] support\n"
    },
    "Pasted image 20221016175942.png": {
        "Backlinking": "![[**Pasted image 20221016175942.png**]]\n",
        "2022-10-16": "![[**Pasted image 20221016175942.png**]]\n"
    },
    "Pasted image 20221010141050.png": {
        "Transactional Globe": "![[**Pasted image 20221010141050.png**]]\n"
    },
    "Pasted image 20221010141055.png": {
        "Transactional Globe": "![[**Pasted image 20221010141055.png**]]\n"
    },
    "Pasted image 20221010141100.png": {
        "Transactional Globe": "![[**Pasted image 20221010141100.png**]]\n"
    },
    "Pasted image 20221010141610.png": {
        "Transactional Globe": "![[**Pasted image 20221010141610.png**]]\n"
    },
    "2022-10-12": {
        "Letter to Mr. Alexander Obenauer on 2022-10-12": "## Reply on [[**2022-10-12**]]\n"
    },
    "2022-10-03": {
        "Letter to Mr. Matt Rickard on 2022-10-03": "- Written: [[**2022-10-03**]]\n"
    },
    "2022-10-04": {
        "Letter to Mr. Matt Rickard on 2022-10-03": "## Follow-up on [[**2022-10-04**]]\n"
    },
    "2022-09-08": {
        "Person A480C6": "- Interviewed at: [[**2022-09-08**]]\n",
        "Person 392196": "- Interviewed at: [[**2022-09-08**]]\n"
    },
    "Person 1B5A5B": {
        "Project PEOPLE": "- [[**Person 1B5A5B**]]\n"
    },
    "Person 88B488": {
        "Project PEOPLE": "- [[**Person 88B488**]]\n"
    },
    "Person 392196": {
        "Project PEOPLE": "- [[**Person 392196**]]\n"
    },
    "Person CC8297": {
        "Project PEOPLE": "- [[**Person CC8297**]]\n",
        "2022-09-13": "- [[**Person CC8297**]]\n"
    },
    "Person 5DBDAE": {
        "Project PEOPLE": "- [[**Person 5DBDAE**]]\n",
        "2022-09-13": "- [[**Person 5DBDAE**]]\n"
    },
    "Person E7CFC5": {
        "Project PEOPLE": "- [[**Person E7CFC5**]]\n",
        "2022-09-14": "  - [[**Person E7CFC5**]]\n"
    },
    "2022-09-06": {
        "Person 88B488": "- Interviewed at: [[**2022-09-06**]]\n",
        "Person 1B5A5B": "- Interviewed at: [[**2022-09-06**]]\n"
    },
    "2022-09-13": {
        "Person 5DBDAE": "- Interviewed at: [[**2022-09-13**]]\n",
        "Person CC8297": "- Interviewed at: [[**2022-09-13**]]\n"
    },
    "people": {
        "Person 5DBDAE": "A lot of [[**people**]] are international.\n",
        "Person E7CFC5": "A lot of smart [[**people**]].\n"
    },
    "2022-09-14": {
        "Person E7CFC5": "- Interviewed at: [[**2022-09-14**]]\n"
    },
    "Atomic Notes System": {
        "Intracranial": "  - [[**Atomic Notes System**]]\n"
    },
    "Infinite Canvas": {
        "Intracranial": "  - [[**Infinite Canvas**]]\n",
        "2022-10-10": "- [[**Infinite Canvas**]]\n"
    },
    "fiercely overpriced": {
        "Autosave Everything": "How would it be [[**fiercely overpriced**]]?\n"
    },
    "2022-10-11": {
        "Open Graph Image as a Service": "- [[**2022-10-11**]]\n"
    },
    "Docusaurus": {
        "Better Docusaurus": "Some things that I currently don't like about [[**Docusaurus**]]:\n"
    },
    "2022-07-18": {
        "Better Docusaurus": "- Render `Figcaption` with `img alt` tags on [[**2022-07-18**]] [Commit](https://github.com/anaclumos/extracranial/commit/cefd8c5b46b2a5c2df2a52e176a97d233387c14b)\n"
    },
    "Pasted image 20220713171801.png": {
        "Digital Addictions of Software Enthusiasts": "![[**Pasted image 20220713171801.png**]]\n"
    },
    "WorkerDOM": {
        "iiframe": "- Does not extend [[**WorkerDOM**]].\n",
        "2022-10-02": "- Got an email from the [[**WorkerDOM**]] team that the project will not have any major updates\n",
        "2022-07-11": "- Replace all unofficial implementation of `getClientBoundingRect` in [[**WorkerDOM**]] with `Element.prototype.getClientBoundingRect()`.\n",
        "2022-08-03": "I did reach a milestone in implementing synchronicity inside [[**WorkerDOM**]], but now I finally realize various advanced use cases exist in",
        "2022-06-14": "- [[**WorkerDOM**]]과 JSDOM은 각자의 한계가 뚜렷하다.\n",
        "2022-06-27": "- Confirm [[**WorkerDOM**]]'s Binary DOM Manipulation channel.\n",
        "2022-07-19": "오늘은 반드시 [[**WorkerDOM**]] [[Scheduler",
        "2022-06-22": "Preferably, [[**WorkerDOM**]] and [[Web Worker",
        "2022-06-26": "- Investigate how [[**WorkerDOM**]] terminates an application. This is a Web Spec.\n",
        "2022-07-12": "### Scheduler|Schedulers in [[**WorkerDOM**]]\n",
        "2022-07-06": "the Technical Proof of Concept and posted here: _PoC that two [[**WorkerDOM**]] upgraded threads don't block each other_ - [Two Roots](https://brane.vercel.app/two-roots-with-infinite-loop/)\n"
    },
    "Cross-Origin-Opener-Policy": {
        "Providing DOM API to Worker Threads": "- [[**Cross-Origin-Opener-Policy**]]: `same-origin`\n"
    },
    "Cross-Origin-Embedder-Policy": {
        "Providing DOM API to Worker Threads": "- [[**Cross-Origin-Embedder-Policy**]]: `require-corp`\n"
    },
    "Pasted image 20220728163149.png": {
        "Providing DOM API to Worker Threads": "![[**Pasted image 20220728163149.png**]]\n"
    },
    "Pasted image 20220728163239.png": {
        "Providing DOM API to Worker Threads": "![[**Pasted image 20220728163239.png**]]\n"
    },
    "Pasted image 20220728170918.png": {
        "Providing DOM API to Worker Threads": "![[**Pasted image 20220728170918.png**]]\n"
    },
    "Pasted image 20220728173011.png": {
        "Providing DOM API to Worker Threads": "![[**Pasted image 20220728173011.png**]]\n"
    },
    "Pasted image 20220729175358.png": {
        "Providing DOM API to Worker Threads": "![[**Pasted image 20220729175358.png**]]\n"
    },
    "Sandbox": {
        "Brane": "  - Prerequisite: [[**Sandbox**]] Environment.\n",
        "2022-06-22": "Then, the next step is to investigate how deep isolation iframe [[**Sandbox**]] provides.\n",
        "2022-06-26": "- Let's not explore ShadowRealm and `iframe` [[**Sandbox**]] for now.\n"
    },
    "2022-07-19": {
        "Developing Review Notes for Obsidian": "일단 오늘([[**2022-07-19**]])은 밤이 늦었으니 내일 계속하도록 한다. 3시간 41분 했다.\n",
        "2022-07-21": "See [[**2022-07-19**]].\n"
    },
    "Better Obsidian": {
        "Developing Review Notes for Obsidian": "[[**Better Obsidian**]] 문서를 보면 알 수 있듯이 별별 아이디어들을 다 쌓아뒀는데 오늘 저녁에",
        "2022-07-10": "- [[**Better Obsidian**]]\n",
        "2022-07-26": "### [[**Better Obsidian**]]\n",
        "2022-07-12": "- [[**Better Obsidian**]]\n"
    },
    "Pasted image 20220719214527.png": {
        "Developing Review Notes for Obsidian": "![[**Pasted image 20220719214527.png**]]\n"
    },
    "Pasted image 20220719220437.png": {
        "Developing Review Notes for Obsidian": "![[**Pasted image 20220719220437.png**]]\n"
    },
    "Pasted image 20220719223820.png": {
        "Developing Review Notes for Obsidian": "![[**Pasted image 20220719223820.png**]]\n"
    },
    "Pasted image 20220719225604.png": {
        "Developing Review Notes for Obsidian": "![[**Pasted image 20220719225604.png**]]\n"
    },
    "Pasted image 20220719225900.png": {
        "Developing Review Notes for Obsidian": "![[**Pasted image 20220719225900.png**]]\n"
    },
    "Pasted image 20220719230101.png": {
        "Developing Review Notes for Obsidian": "![[**Pasted image 20220719230101.png**]]\n"
    },
    "Pasted image 20220719231900.png": {
        "Developing Review Notes for Obsidian": "![[**Pasted image 20220719231900.png**]]\n"
    },
    "Pasted image 20220719232028.png": {
        "Developing Review Notes for Obsidian": "![[**Pasted image 20220719232028.png**]]\n"
    },
    "Pasted image 20220726204531.png": {
        "Developing Review Notes for Obsidian": "![[**Pasted image 20220726204531.png**]]\n"
    },
    "Logseq": {
        "Better Obsidian": " - List view for Daily Notes like Roam Research and [[**Logseq**]].\n"
    },
    "Internetworking": {
        "2022-08-28": "- [[**Internetworking**]]\n",
        "2022-08-29": "- [[**Internetworking**]]\n"
    },
    "조선왕조실록": {
        "2022-08-28": "- [[**조선왕조실록**]]\n"
    },
    "사회과부도": {
        "2022-08-28": "- [[**사회과부도**]]\n"
    },
    "Ticket Bounty Model": {
        "2022-09-12": "- [[**Ticket Bounty Model**]]\n",
        "2022-08-11": "- [[**Ticket Bounty Model**]]\n"
    },
    "Nation as a Service": {
        "2022-09-12": "- [[**Nation as a Service**]]\n"
    },
    "Ignoring $ for copy-pasting online resources to terminal": {
        "2022-09-12": "- [[**Ignoring $ for copy-pasting online resources to terminal**]]\n"
    },
    "DOM": {
        "2022-07-11": "This giant object has lots of APIs which look exactly like [[**DOM**]] APIs.\n"
    },
    "Pasted image 20220711010013.png": {
        "2022-07-11": "![[**Pasted image 20220711010013.png**]]\n"
    },
    "Pasted image 20220711013828.png": {
        "2022-07-11": "![[**Pasted image 20220711013828.png**]]\n"
    },
    "2022-07-06": {
        "2022-07-11": "Confirmed a demo script created on [[**2022-07-06**]] yields logs as above.\n"
    },
    "Pasted image 20220711014245.png": {
        "2022-07-11": "![[**Pasted image 20220711014245.png**]]\n"
    },
    "Scheduler": {
        "2022-07-11": "- The hard part is transports and [[**Scheduler**|schedulers]].\n",
        "2022-07-19": "2022-07-11에 확인한 2가지 [[**Scheduler**|스케줄러]]를 확인한다.\n",
        "2022-07-12": "### [[**Scheduler**|**Scheduler**s]] in [[WorkerDOM"
    },
    "DOM Reflow": {
        "2022-07-11": "because it depends on values that could only be found after [[**DOM Reflow**]].\n"
    },
    "cooperative multitasking": {
        "2022-07-11": "Brane should use either of the above to attain [[**cooperative multitasking**]].\n"
    },
    "preemptive multitasking": {
        "2022-07-11": "However, a guest app should look as [[**preemptive multitasking**]] as if blocking APIs blocks a thread.\n"
    },
    "Atomics": {
        "2022-07-11": "To automatically use [[**Atomics**]] API for every third-party app, [[Brane",
        "2022-08-03": "This happens synchronously using [[**Atomics**]] and [[SharedArrayBuffer",
        "2022-06-27": "- Learn more about [[**Atomics**]] and [[SharedArrayBuffer"
    },
    "Better Docusaurus": {
        "2022-07-11": "- [[**Better Docusaurus**]]\n",
        "2022-07-18": "- [[**Better Docusaurus**]]: Render `Figcaption` with `img alt` tags. [Commit](https://github.com/anaclumos/extracranial/commit/cefd8c5b46b2a5c2df2a52e176a97d233387c14b)\n",
        "2022-07-12": "- [[**Better Docusaurus**]]\n"
    },
    "P vs NP": {
        "2022-07-01": "- [[**P vs NP**]]\n"
    },
    "Dynamic Island on the Web": {
        "2022-10-12": "- [[**Dynamic Island on the Web**]]\n",
        "2022-10-16": "### [[**Dynamic Island on the Web**]]\n",
        "2022-10-15": "- [[**Dynamic Island on the Web**]]\n"
    },
    "Letter to Mr. Alexander Obenauer on 2022-10-12": {
        "2022-10-12": "- [[**Letter to Mr. Alexander Obenauer on 2022-10-12**]]\n"
    },
    "Cosmic Noises of Life": {
        "2022-10-12": "- [[**Cosmic Noises of Life**]]\n"
    },
    "Jetbrains Fleet": {
        "2022-10-12": "- [[**Jetbrains Fleet**]] is [[GA"
    },
    "GA": {
        "2022-10-12": "- Jetbrains Fleet is [[**GA**]]: [Introducing the Fleet Public Preview](https://blog.jetbrains.com/fleet/2022/10/introducing-the-fleet-public-preview)\n"
    },
    "iOS Style Toggle in CSS": {
        "2022-10-12": "  - [x] Archived [[**iOS Style Toggle in CSS**]]\n"
    },
    "월급만큼 중요한 것은 자존급이다": {
        "2022-10-06": "- [[**월급만큼 중요한 것은 자존급이다**]]\n"
    },
    "Cube": {
        "2022-08-18": "- [[**Cube**]]\n",
        "2022-10-13": "- [[**Cube**]]\n"
    },
    "Microproject": {
        "2022-08-18": "- [[**Microproject**]]\n"
    },
    "Undefined symbol vtable": {
        "2022-09-22": "- [[**Undefined symbol vtable**]]\n"
    },
    "Pasted image 20220721160016.png": {
        "2022-07-21": "![[**Pasted image 20220721160016.png**]]\n"
    },
    "Pasted image 20220721160612.png": {
        "2022-07-21": "![[**Pasted image 20220721160612.png**]]\n"
    },
    "Quoridor Game": {
        "2022-07-31": "- [[**Quoridor Game**]]\n"
    },
    "Tesla Release Model": {
        "2022-09-06": "- [[**Tesla Release Model**]]\n"
    },
    "Better Aldehyde": {
        "2022-10-16": "### [[**Better Aldehyde**]]\n",
        "2022-07-12": "- [[**Better Aldehyde**]]\n"
    },
    "AA04D0.gif": {
        "2022-10-16": "![[**AA04D0.gif**]]\n"
    },
    "Kakao": {
        "2022-10-16": "### [[**Kakao**]]\n"
    },
    "How is WebAssembly cross-platform when Assembly is not": {
        "2022-10-16": "- [ ] Finalize [[**How is WebAssembly cross-platform when Assembly is not**]]\n",
        "2022-09-20": "- [[**How is WebAssembly cross-platform when Assembly is not**]]\n"
    },
    "Markdown Email Client": {
        "2022-10-16": "- [ ] Review [MailPilot](https://www.mailpilot.app/) [[**Markdown Email Client**]]\n",
        "2022-10-04": "- [[**Markdown Email Client**]]\n"
    },
    "Pragmatic Engineer": {
        "2022-07-14": "## Today's [[**Pragmatic Engineer**]]\n",
        "2022-06-21": "## Today's [[**Pragmatic Engineer**]]\n"
    },
    "Redirects": {
        "2022-09-17": "- [[**Redirects**]]\n"
    },
    "Android": {
        "2022-09-17": "- [[**Android**]]\n"
    },
    "Readings": {
        "2022-09-07": "- [[**Readings**]]\n"
    },
    "sem.sh": {
        "2022-10-03": "- [[**sem.sh**]]\n"
    },
    "Ideas are worthless": {
        "2022-10-03": "- [[**Ideas are worthless**]]\n"
    },
    "Alter": {
        "2022-10-03": "- [[**Alter**]]\n"
    },
    "Screenshot as an API": {
        "2022-10-03": "- Moved to [[**Screenshot as an API**]]\n",
        "2022-10-10": "- [PDF processing and analysis with open-source tools](https://www.bitsgalore.org/2021/09/06/pdf-processing-and-analysis-with-open-source-tools). This looks terrifying. [[**Screenshot as an API**]] all the way!\n"
    },
    "Realm Shim": {
        "2022-05-30": "- What is [[**Realm Shim**]]?\n"
    },
    "Either be the best-in-class or the most efficient": {
        "2022-10-13": "- [[**Either be the best-in-class or the most efficient**]]\n"
    },
    "Cross-platform AirDrop": {
        "2022-10-13": "- [Taildrop · Tailscale](https://tailscale.com/kb/1106/taildrop/): [[**Cross-platform AirDrop**]]\n"
    },
    "Jumpsite": {
        "2022-10-13": "- [x] [[**Jumpsite**]]\n"
    },
    "ACSM": {
        "2022-06-05": "- _Convert [[**ACSM**]] files to [[PDF"
    },
    "UUIDs are Awesome": {
        "2022-09-28": "- [[**UUIDs are Awesome**]]\n"
    },
    "일본 과학기술 총력전": {
        "2022-08-02": "- [[**일본 과학기술 총력전**]]\n",
        "Readings": "- [x] [[**일본 과학기술 총력전**]]\n"
    },
    "nextjs": {
        "2022-09-29": "  - [[**nextjs**]] framework\n"
    },
    "The New Era of macOS Applications": {
        "2022-09-29": "- [[**The New Era of macOS Applications**]]\n"
    },
    "Inspiring Open Source Projects": {
        "2022-09-29": "- [[**Inspiring Open Source Projects**]]\n"
    },
    "한자 타자기": {
        "2022-09-29": "- [[**한자 타자기**]]\n",
        "Readings": "- [ ] [[**한자 타자기**]]\n"
    },
    "Pasted image 20220803145329.png": {
        "2022-08-03": "![[**Pasted image 20220803145329.png**]]\n"
    },
    "Pasted image 20220803210537.png": {
        "2022-08-03": "![[**Pasted image 20220803210537.png**]]\n"
    },
    "165F8C.jpeg": {
        "2022-08-03": "![[**165F8C.jpeg**]]\n"
    },
    "Pasted image 20220803112747.png": {
        "2022-08-03": "![[**Pasted image 20220803112747.png**]]\n"
    },
    "iiframe": {
        "2022-08-03": "One straightforward use case [[**iiframe**]] — improved iframe — that specializes in containerizing iframes in [[Web"
    },
    "Academics": {
        "2022-08-23": "- Added Bunch of [[**Academics**]]\n"
    },
    "Apple Earth": {
        "2022-08-23": "- [[**Apple Earth**]]\n"
    },
    "Someday": {
        "2022-09-09": "- [[**Someday**]]\n"
    },
    "Typescript": {
        "2022-09-09": "- It's a **stand-alone [[**Typescript**]]+Rust/WASM library** with its own React-like run-time. If you're familiar with React,"
    },
    "Optimistic Nihilism": {
        "2022-08-10": "- [[**Optimistic Nihilism**]]\n",
        "2022-08-04": "### [[**Optimistic Nihilism**]]\n"
    },
    "원숭이와 꽃신": {
        "2022-08-10": "- [[**원숭이와 꽃신**]]\n"
    },
    "CalliFontia": {
        "2022-08-10": "- [[**CalliFontia**]]\n",
        "2022-08-11": "- [[**CalliFontia**]]\n"
    },
    "Separation of Computer Scientists and Computer Technologists": {
        "2022-08-24": "- [[**Separation of Computer Scientists and Computer Technologists**]]\n"
    },
    "Providing DOM API to Worker Threads": {
        "2022-07-29": "- [[**Providing DOM API to Worker Threads**]] Complete\n",
        "2022-07-28": "- [[**Providing DOM API to Worker Threads**]]\n"
    },
    "Digital Addictions of Software Enthusiasts": {
        "2022-07-29": "  - [[**Digital Addictions of Software Enthusiasts**]]\n",
        "2022-07-13": "- [[**Digital Addictions of Software Enthusiasts**]]\n"
    },
    "2022-07-11": {
        "2022-07-19": "[[**2022-07-11**]]에 확인한 2가지 [[Scheduler|스케줄러",
        "2022-07-12": "On [[**2022-07-11**]], I analyzed that [[Brane"
    },
    "Autosave Everything": {
        "2022-07-19": "- [[**Autosave Everything**]]\n"
    },
    "Developing Review Notes for Obsidian": {
        "2022-07-19": "- [[**Developing Review Notes for Obsidian**]]\n",
        "2022-07-26": "- [[**Developing Review Notes for Obsidian**]]\n"
    },
    "markdown": {
        "2022-07-09": "- I worked on a plugin for converting wikilinks to [[**markdown**]] links. For example, this is a wikilink to [[Hacker News"
    },
    "Pasted image 20220804181751.png": {
        "2022-08-04": "![[**Pasted image 20220804181751.png**]]\n"
    },
    "Pasted image 20220804003204.png": {
        "2022-08-04": "![[**Pasted image 20220804003204.png**]]\n"
    },
    "InterviewKit": {
        "2022-08-30": "- [[**InterviewKit**]]\n"
    },
    "WeCrashed": {
        "2022-07-18": "- Finished [[**WeCrashed**]]\n"
    },
    "Minimaximalism": {
        "2022-07-18": "- [[**Minimaximalism**]]: Let's remove every unnecessaries.\n"
    },
    "UBI for Scholars": {
        "2022-07-18": "- [[**UBI for Scholars**]]\n"
    },
    "Blitzscaling": {
        "2022-06-06": "- [[**Blitzscaling**]] will not work, starting now. User Growth ≠ Valuation.\n"
    },
    "356 Official SSH Server": {
        "2022-08-25": "- [[**356 Official SSH Server**]]\n"
    },
    "sandbox": {
        "2022-08-01": "- Maybe running something like the QuickJS [[**sandbox**]] inside WebAssembly can provide a more secure way to access selective"
    },
    "D4C125.gif": {
        "2022-08-01": "![[**D4C125.gif**]]\n"
    },
    "서울 프로": {
        "2022-08-01": "- [[**서울 프로**]]\n"
    },
    "Inliner": {
        "2022-10-04": "- [[**Inliner**]]\n"
    },
    "Search Engine Indexing Your Life": {
        "2022-10-04": "- [[**Search Engine Indexing Your Life**]]\n"
    },
    "Try Removing": {
        "2022-10-04": "- [[**Try Removing**]]\n"
    },
    "Lingua Franca": {
        "2022-09-30": "- [[**Lingua Franca**]]\n"
    },
    "45E559.jpeg": {
        "2022-10-14": "![[**45E559.jpeg**]]\n"
    },
    "0593F3.jpeg": {
        "2022-10-14": "![[**0593F3.jpeg**]]\n"
    },
    "Pasted image 20221014172732.png": {
        "2022-10-14": "![[**Pasted image 20221014172732.png**]]\n"
    },
    "BIMI": {
        "2022-10-14": "- [[**BIMI**]]\n"
    },
    "Linear": {
        "2022-10-14": "  - [[**Linear**]] has been popping up on my Twitter threads.\n"
    },
    "Linear README": {
        "2022-10-14": "  - Especially [[**Linear README**]]\n"
    },
    "Pasted image 20221010232130.png": {
        "2022-10-10": "    - ![[**Pasted image 20221010232130.png**]]\n"
    },
    "Super App and Democracy": {
        "2022-10-10": "- [[**Super App and Democracy**]]\n",
        "2022-10-11": "- [[**Super App and Democracy**]]\n"
    },
    "Tools I will use one day": {
        "2022-10-10": "- [[**Tools I will use one day**]]. Found [tremor](https://www.tremor.so/).\n"
    },
    "ActivityPub": {
        "2022-10-10": "- [[**ActivityPub**]]\n"
    },
    "Firefox": {
        "2022-10-10": "responsiveness on macOS - Mozilla Hacks - the Web developer blog](https://hacks.mozilla.org/2022/10/improving-firefox-responsiveness-on-macos/) [[**Firefox**]]\n"
    },
    "2971DF.jpeg": {
        "2022-07-12": "![[**2971DF.jpeg**]]\n"
    },
    "Off The Record": {
        "2022-09-11": "- [[**Off The Record**]]\n"
    },
    "Letter to the Heptabase Team on 2022-10-11": {
        "2022-10-11": "- [[**Letter to the Heptabase Team on 2022-10-11**]]\n"
    },
    "Research": {
        "2022-10-11": "on a Journal Page, I should start separating them into individual [[**Research**]] pages.\n"
    },
    "1F98FE.jpeg": {
        "2022-07-06": "- Merged! ![[**1F98FE.jpeg**]]\n"
    },
    "Guestbook": {
        "2022-10-15": "- [[**Guestbook**]]\n"
    },
    "3D Globe on the Web": {
        "2022-10-15": "- [[**3D Globe on the Web**]]: [Low Earth Orbit Visualization | LeoLabs](https://platform.leolabs.space/visualization)\n"
    },
    "경제 개발의 길목에서": {
        "Readings": "- [x] [[**경제 개발의 길목에서**]]\n"
    },
    "Atomic Habits": {
        "Readings": "- [ ] [[**Atomic Habits**]]\n"
    },
    "지구 끝의 온실": {
        "Readings": "- [x] [[**지구 끝의 온실**]]\n"
    },
    "2022-09-29": {
        "한자 타자기": "- Started reading [[**2022-09-29**]]\n"
    }
}