...
const highlightClasses = ['highlight-1', 'highlight-2', 'highlight-3', 'highlight-4', 'highlight-5', 'highlight-6']

export const Highlight = ({ trigger, title, content, direction = 'bottom' }: HighlightProps) => {
  ...
  return (
    <>
      <span className="hidden md:inline-block">
        <Popover>
          <PopoverTrigger className={cn('cursor-pointer inline-block relative', highlightClass)} type="button">
            {trigger}
          </PopoverTrigger>
          <PopoverContent className="w-96 md:w-96" side={direction} align="center">
            <div className="space-y-2">
              {title && (typeof title === 'string' ? <h4 className="text-lg font-bold">{title}</h4> : title)}
              {content}
            </div>
          </PopoverContent>
        </Popover>
      ...
      <span className="md:hidden">
        <Drawer.Root>
          <Drawer.Trigger className={cn('cursor-pointer inline-block relative', highlightClass)} type="button">
            {trigger}
          </Drawer.Trigger>
          <Drawer.Portal>
