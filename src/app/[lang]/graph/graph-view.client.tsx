...
const GraphViewLazy = lazy(async () => {
  ...
  function Component({
    ...
    const handleNodeHover = useCallback((node: Node | null) => {
      ...
      if (node && typeof node.x === 'number' && typeof node.y === 'number') {
        ...
        setTooltip({
          x: coords.x + 4,
          y: coords.y + 4,
          content: (node as any).description ?? 'No description',
        })
      } else {
        setTooltip(null)
      }
    ...
    const nodeCanvasObject: ForceGraphProps['nodeCanvasObject'] = (node: any, ctx) => {
      ...
      const hoverNode = hoveredRef.current as any
    ...
    const enrichedNodes = useMemo(() => {
      const enrichedNodes = nodes.map((node: any) => ({
        ...node,
        neighbors: links.flatMap((link: any) => {
          if (link.source === node.id) return link.target as any
          if (link.target === node.id) return link.source as any
          return []
        }),
      }))
...
export function GraphViewClient(props: GraphViewProps) {
  ...
  return (
    <div ref={ref} className="relative border h-[600px] [&_canvas]:size-full rounded-xl overflow-hidden">
